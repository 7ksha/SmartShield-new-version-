1.1.16 (Dec 1st, 2025)
- Fix problem reporting evidence when smartshield is monitoring one interface
- Change the usage of -g option, now smartshield requires the interface name to monitor when using -g.
...
- Handle smartshield and iptables failovers when running smartshield as an access point in the Raspberry Pi.


1.1.15 (Oct 31st, 2025)
- Fix FP connection to port 0 for IGMP flows.
- Support monitoring two interfaces when smartshield is running as an access point.
- Improve running smartshield on a growing zeek directory (using -g): smartshield can now detect the interface, host IP and gateway IP.
...
- Speedup of evidence processing in smartshield.
...
- Avoid reporting own self or other smartshield peers when arp poisoning attackers.
...
- Support running smartshield as an AP to block attackers in the RPI.
...
- Fix the issue of smartshield hanging when shutting down.
...
- Support reading labeled Zeek logs and using their labels in smartshield modules.
...
- Add the option to enable/disable local and online whitelists from smartshield.yaml.
- Fix false positive "Connection to a private IP outside of local network" detection. smartshield now doesn't alert on DNS servers outside of local network.
...
- Improve "DNS without connection" evidence. smartshield now only detects when the query type is A or AAAA.
...
- Improve whitelists: smartshield is now whitelisting CNAME, SNI, related queries, and DNS resolutions of attackers and victims.
...
* Fix regex warning when starting smartshield. Special thanks to @Sekhar-Kumar-Dash.
...
- Add support to IP ranges as the client_ip in smartshield.yaml
...
- Update smartshield internal list of Apple known ports.

1.1.4.1 (Dec 3rd, 2024)
- Fix abstract class starting with the rest of the modules.
- Fix the updating of the MAC vendors database used in smartshield.
...
- Add support for a light smartshield docker image.

1.1.3 (October 30th, 2024)
- Enhanced smartshield shutdown process for smoother operations.
- Optimized resource management in smartshield, resolving issues with lingering threads in memory.
- Remove the progress bar; smartshield now provides regular statistical updates.
...
- Print more information to CLI on smartshield startup, including network details, client IP, thresholds used, and more.
...
- Discontinued MacOS m1 docker images, P2p images, and smartshield dependencies image.
- Fix the problem of the progress bar stopping before analysis is done, causing smartshield to freeze when analyzing large PCAPs.
- Improve how smartshield recognizes the current host IP.
- Increase the speed of the Flowalerts module by changing how smartshield checks for DNS servers.
...
- Fix smartshield installation scripts at install/install.sh
- Improve how smartshield validates domains taken from TI feeds.
- Improve whitelists.
...
1.1 (July 2024)
- Update Python version to 3.10.12 and all python libraries used by smartshield.
- Update nodejs and zeek.
- Improve the stopping of smartshield. Modules now have more time to process flows.
- Fix database unit tests overwriting config/redis.conf.
- New config file format, smartshield is now using yaml thanks to @patel-lay.
- Better unit tests. thanks to @Sekhar-Kumar-Dash.
...
- More minimal smartshield notifications, now smartshield displays the alert description instead of all evidence in the alert.
- The port of the web interface is now configurable in smartshield.conf
...
- Improve and compress smartshield Docker images. thanks to @verovaleros
...
1.0.12 (March 2024)
- Add an option to specify the current client IP in smartshield.conf to help avoid false positives.
- Better handling of URLhaus threat intelligence.
- Change how smartshield determines the local network of the current client IP.
...
- Fix problem releasing the terminal when smartshield is done.
...
- Fix stopping smartshield daemon.
...
- Remove the home_network feature from smartshield.
...
- Add an option in smartshield.conf to wait for the update manager to update all TI feeds before starting the rest of smartshield to avoid missing any blacklisted IPs.
- Fix error detecting password guessing.
- Fix issues reading all flows when running on a low-spec device.
- Improved the stopping of smartshield and termination of processes.
...
- Add support for exporting smartshield alerts to a sqlite db


-1.0.6 (June 2023):
- Store flows in SQLite database in the output directory instead of redis.
- 55% RAM usage decrease.
- Support the labeling of flows based on smartshield detections.
...
- Print the number of evidence generated by smartshield when running on PCAPs and interface.
...
- Fix problem starting smartshield in docker in linux.
- Ignore ICMP scans if the flow has ICMP type 3
- Improve our whitelist. smartshield now checks for whitelisted attackers and victims in the generated evidence.
...
- Remove the feature of creating log directories using -l, now the only logs smartshield generates are store in the output/ directory.
...
- Update smartshield known ports.
...
- Update smartshield default whitelist.
...
- Instead of dos detection. smartshield is now detecting all executables thanks to @Onyx2406
...
- Add a progress bar to smartshield showing the number of processed flows
...
- Fix race condition trying to update TI files when running multiple smartshield instances
...
- P2P can now work without adding the p2p4smartshield binary to PATH
...
- Remove rstcloud TI file from smartshield.conf
- Add the option to change pastebin download detection threshold in smartshield.conf
- Add the option to change shannon entropy threshold detection threshold in smartshield.conf
...
- Add the option to start smartshield web interface automatically using -w
...
- Add -g option for running smartshield on growing zeek dirs. (for example dirs generated by zeek running on an interface)
...
- Don't stop smartshield when p2p is enabled but smartshield is given a file, not an interface.
- Add Detection of SSH password guessing by smartshield in addition to zeek.
...
- Support running smartshield on a growing zeek dir. for example a zeek dir of an interface.
...
- Kill smartshield on  when redis ConnectionError occurs
...
- Support using multiple home network parameters in smartshield.conf
- Add redis.conf for special redis configurations when running smartshield
...
- Add the option to close all redis servers when smartshield can't start because all port are unavailable
- Remove support for whitelisting an unsupported org by smartshield
- Better description of alerts exported to Slack
- Faster Whitelists
- Whitelist connections made by smartshield causing false positives
...
- Fix running several smartshield instances
...
-0.9.3 (July 2022)
* smartshield
- Run multiple smartshield instances on demand using (-m), and use redis port 6379 by default.
- Fix false positive 'DNS resolution without connection' alerts
- Faster smartshield and reduced memory and CPU consumption
...
- Fix problem stopping input process when smartshield stops
- Update the locations of GeoIP database in zeek for better zeek detections
- Fix P2P output dir, now it's the same as alerts.log and smartshield.log
- Update our usage of macvendors.com API
- Whitelist the connections made by smartshield, so now you won't be alerted when smartshield is using virustotal.com or macvendors.com
...
- Fix problem stopping smartshield when using whitelists
...
- Added support for running smartshield in the background as a daemon
- Fix the issue of growing zeek logs by deleting old zeek logs every 1 day. (optional but enabled by default)
- Added support for running several instances of smartshield at the same time.
- Saving and loading the db in macos
- Fix reading flows from stdin, now it supports zeek, argus and suricata
- Faster Startup of smartshield, now smartshield updates the TI files in the background
- Added smartshield.log where all smartshield logs goes. in daemon and interactive mode
...
* smartshield
 ...
 - Detect multiple SSH client versions. smartshield will now alert if an IP is detected using OpenSSH_8.1 then OpenSSH_7.1 for example
 ...
 - Don't create p2p.log by default, unless create_p2p_log is set to yes in smartshield.conf
-0.8.5 (March 2022)
* smartshield
  ...
  - Support deleting of large log files (arp.log) in case the user doesn't want a copy of the log files after smartshield is done
  ...
  - Added a zeek script to recognize DoH flows for more real-time experience while using smartshield
  - Change the structure of smartshield files by splitting large modules into smaller files.
  ...
  - Document all the internet connections made by smartshield
  - Fix install.sh
  - Add errors.log to output/ dir to log errors encountered by smartshield.
-0.8.4 (Feb 2022)
* smartshield
  ...
  - Faster startup of smartshield, now TI files are updated concurrently.
  - Add a logstash configuration file to allow exporting smartshield alerts.
  ...
  - Add "Blocked by smartshield" comment to all iptables rules added by smartshield
  - Improve whitelisting by updating organizations' domains.
  - Update documentation
  - Fix invalid JSON alerts in alerts.json
  - Fix problem stopping smartshield.
...
*	smartshield
		...
		- Detect DGA
		- Support popup notifications in Linux and mac. disabled by default. enable it by changing popup_alerts to yes in smartshield.conf
		...
		- Add support for enabling and disabling detections in smartshield.conf
		- Read RiskIQ email and API key from modules/RiskIQ/credentials instead of the configuration file.
		- Now log files are disabled by default, use -l or set create_log_files to yes in smartshield.conf for enabling them.
		- Support commenting TI files in smartshield.conf: when TI files are commented using ; in smartshield.conf, they are completely removed from our database.
		- Now smartshield generates alerts in IDEA format by default in alerts.json
...
- 0.8
    - smartshield
        ...
        - smartshield now allows you to re-train the machine learning model for flows with your own traffic. You can extend the current model, or start from scratch.
		...
		- Be able to run multiple independent instances of smartshield in the same machine.
		- Save and load redis databases to disk as backup for later analysis.
		- Add unit tests in tests/ folder.
        - Use our own Zeek configuration file, so smartshield does not collide with the local installation.
        - Use our own Zeek scripts folder, so smartshield does not collide with the local installation.
...
- 0.7.1 (published 2020/12/18)
	- smartshield
		- fix the function of smartshield to stop after pressing CTRL-C
		- fix smartshield stopping automatically
		...
		- add 3 new TI feeds to smartshield conf by default
	...
	- Kalipso
		...
		- execution of Kalipso from smartshield folder: $./kalipso.sh
...
- 0.7.0 (published 2020/09/25)
	- smartshield
		...
		- Default label of the flow in the smartshield.conf is changed to 'unknown'
		- Added a module to block IPs when running smartshield on interface in Linux machine
...
- 0.6.9 (published 2020/08/12)
	- smartshield
		...
		- Added parameters in smartshield.conf for updating VirusTotal and Threat Intelligence
		- Added new channel 'core_messages', UpdateManager is subscribed to that
		- Added manager to search host IP, if smartshield is running on interface and networks are changing
...
- 0.6.6
	...
	- Changed the timeline format in smartshield and Kalipso
	- Defined host IP in smartshield and Kalipso if smartshield is run on interface
- 0.6.5
	- Fixed Threat Intelligence module to be fully functional.
	- Added new feature to stop smartshield automatically when input files ends.
...
- 0.3.4
	...
	- Now we have a configuration file smartshield.conf. In there you can specify from fixed parameters, the time formats, to the columns in the flow file.
