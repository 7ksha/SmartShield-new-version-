...
This script will:
 ...
 - Run smartshield inside Docker + tmux
 - Log smartshield & Docker status to smartshield_container.log
 - Create a systemd unit for smartshield for persistence
EOF
}
...
create_directories() {
  ...
  echoc "${GREEN}This script will mount ${OUTPUT_DIR} into the Docker container as smartshield output directory in /StratosphereLinuxIPS/output.\n ${RESET}"
}
...
create_smartshield_runner_script() {
  # Creates the smartshield-runner.sh script from template smartshield-runner-template.sh
  RUNNER_PATH="/usr/local/bin/smartshield-runner.sh"
  TEMPLATE="./smartshield-runner-template.sh"
  LOG_FILE="${CWD}/smartshield_container.log"

  echoc "${BLUE}Creating runner script from template for smartshield systemd unit to use...${RESET}"
  [ -f "$TEMPLATE" ] || { echoc "${RED}Template not found: $TEMPLATE${RESET}"; exit 1; }
  echoc "PS: This smartshield runner script doesn't start smartshield with the blocking modules enabled, modify the smartshield command in ${TEMPLATE}
...
create_smartshield_systemd_unit() {
  SERVICE_PATH="/etc/systemd/system/smartshield.service"
  TEMPLATE="./smartshield.service.template"

  echoc "${BLUE}Creating smartshield systemd service from template ./smartshield.service.template ...${RESET}"
  [ -f "$TEMPLATE" ] || { echoc "${RED}Template not found: $TEMPLATE${RESET}"; exit 1; }
  ...
  export CONTAINER_NAME="smartshield"
  export DOCKER_IMAGE="stratosphereips/smartshield:latest"
  export RUNNER_PATH="/usr/local/bin/smartshield-runner.sh"

  envsubst < "$TEMPLATE" > "$SERVICE_PATH"

  systemctl daemon-reload
  systemctl enable smartshield.service
  systemctl restart smartshield.service
  echoc "${GREEN}smartshield systemd service installed and started.${RESET}"
  echoc "${BOLD}You can check the status with: ${RESET} sudo systemctl status smartshield\n"
}
...
main() {
  ...
  echoc "${YELLOW}smartshield is running inside tmux in Docker.${RESET}"
  echoc "You can attach using: ${BOLD}docker exec -it smartshield${RESET}"
  echoc "For container logs check: ${BOLD}${CWD}/smartshield_container.log${RESET}"
}
