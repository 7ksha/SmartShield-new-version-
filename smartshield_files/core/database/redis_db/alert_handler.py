...
class AlertHandler:
    ...
    def mark_as_ot_protected(self, ip: str):
        """
        Mark an IP as an OT-protected device so blocking is suppressed.
        Used by the OT Detection module to tag PLCs, HMIs, SCADA servers.
        Stored in Redis set 'ot_protected_ips'.
        """
        self.r.sadd("ot_protected_ips", ip)

    def is_ot_protected(self, ip: str) -> bool:
        """Returns True if ip is in the OT-protected devices set."""
        return bool(self.r.sismember("ot_protected_ips", ip))

    def get_ot_protected_ips(self) -> set:
        """Returns the set of all OT-protected IPs."""
        return {m.decode() if isinstance(m, bytes) else m
                for m in self.r.smembers("ot_protected_ips")}
