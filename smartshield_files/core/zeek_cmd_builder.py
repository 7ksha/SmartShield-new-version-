...
class ZeekCommandBuilder:
    ...
    def _get_rotation_args(self) -> List[str]:
        ...
        if self.input_type == "interface" and self.enable_rotation:
            # how often to rotate zeek files? taken from smartshield.yaml
            return [
                "-e",
    ...
    def _build_packet_filter(self, tcpdump_filter: Optional[str]) -> List[str]:
        # build packet filter
        # user-defined filter in smartshield.yaml
        packet_filter = (
    ...
    def _get_relative_pcap_path(self, pcap: str) -> str:
        ...
        if not os.path.isabs(pcap):
            # now the given pcap is relative to smartshield main dir
            # smartshield can store the zeek logs dir either in the
            # output dir (by default in smartshield/output/<filename>_<date>/zeek_files/),
            # or in any dir specified with -o
            # construct an abs path from the given path so smartshield can find the given pcap
