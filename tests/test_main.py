...
# def test_get_smartshield_version():
#     main = ModuleFactory().create_main_obj()
...
#     ) as mock_file:
#         result = main.get_smartshield_version()
#
...
def test_check_zeek_or_bro_not_found():
    ...
    with (
        ...
        patch.object(main, "terminate_smartshield") as mock_terminate,
    ):
        result = main.check_zeek_or_bro()
...
def test_terminate_smartshield_interactive():
    main = ModuleFactory().create_main_obj()
    main.mode = "interactive"
    main.conf = MagicMock()
    ...
    with patch.object(sys, "exit") as mock_exit:
        main.terminate_smartshield()

    mock_exit.assert_called_once_with(0)


def test_terminate_smartshield_daemonized():
    main = ModuleFactory().create_main_obj()
...
def test_terminate_smartshield_cpu_profiler_enabled():
    main = ModuleFactory().create_main_obj()
    main.mode = "interactive"
    main.conf = MagicMock()
...
def test_prepare_output_dir_with_o_flag_deletes_files_preserve_smartshield(
    main_obj, tmp_path, monkeypatch
):
    test_dir = Path(main_obj.args.output)
    test_dir.mkdir(parents=True)
    (test_dir / "a.txt").write_text("remove")
    (test_dir / "smartshield_output.txt").write_text("keep")
    ...
    assert "smartshield_output.txt" in remaining
...
@patch(
    "smartshield_files.common.smartshield_utils.utils.convert_ts_format",
