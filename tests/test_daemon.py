...
@pytest.mark.parametrize(
    ...
    [  # testcase1: Create all streams
        (
            ...
            "smartshield.log",
            "smartshield.log",
            ["errors.log", "smartshield.log"],
        ),
        # testcase2: Create only stderr when stopping
        (["-S"], "errors.log", "smartshield.log", "smartshield.log", ["errors.log"]),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [  # testcase1: Using /var/log/smartshield/ directory
        (
            "/var/log/smartshield/",
            "/var/log/smartshield/errors.log",
            "/var/log/smartshield/smartshield.log",
            "/var/log/smartshield/smartshield.log",
        ),
        # testcase2: Using custom output directory
        (
            "/tmp/smartshield",
            "/tmp/smartshield/errors.log",
            "/tmp/smartshield/smartshield.log",
            "/tmp/smartshield/smartshield.log",
        ),
    ],
)
...
def test_get_last_opened_daemon_info(file_content, expected_result):
    daemon = ModuleFactory().create_daemon_object()
    daemon.smartshield.redis_man.running_logfile = "mock_logfile.txt"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, ["pidfile deleted."], [call("/tmp/smartshield_daemon.lock")]),
        # Test case 2: pidfile doesn't exist
        (
            False,
            [
                "Can't delete pidfile, /tmp/smartshield_daemon.lock doesn't exist.",
