...
def test_log_redis_server_pid_normal_ports(
    ...
    smartshield_files.common.smartshield_utils.utils.convert_ts_format = Mock(
        return_value="Date"
    )
...
def test_load_db_success(mock_db):
    ...
    redis_manager.main.terminate_smartshield = Mock()
    ...
    with (
        ...
        redis_manager.main.terminate_smartshield.assert_called_once()
...
def test_check_redis_database_failure(mock_db):
    ...
    with (
        ...
        patch("os.system") as mock_system,
        patch("time.sleep") as mock_sleep,
        patch.object(redis_manager.main, "terminate_smartshield") as mock_terminate,
    ):
        result = redis_manager.check_redis_database()
...
@pytest.mark.parametrize(
    "port",
    [
        # Testcase 1: Using the starting port of smartshield range
        32768,
        # Testcase 2: Another port within smartshield range
        32769,
...
def test_print_port_in_use(port, mock_db):
    ...
    with patch("builtins.print") as mock_print:
        ...
        expected_output = (
            ...
            f"Or kill your open redis ports using: ./smartshield.py -k "
        )
...
def test_remove_old_logline(
    ...
    with (
        ...
        mocked_open.assert_any_call("tmp_running_smartshield_log.txt", "w")
        ...
        mock_replace.assert_called_once_with(
            "tmp_running_smartshield_log.txt", redis_manager.running_logfile
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "Logs Dir, smartshield PID, Is Daemon, Save the DB"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Testcase 2: Empty file
        ("", "No open redis servers in running_smartshield_info.txt", {}),
    ],
)
