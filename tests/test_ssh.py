...
from smartshield_files.core.flows.zeek import SSH
...
@pytest.mark.parametrize(
    "auth_success, expected_zeek_evidence, expected_called_smartshield",
    [
        ...
        # - should call detect_successful_ssh_by_smartshield
        ("false", False, True),
...
async def test_check_successful_ssh(
    mocker, auth_success, expected_zeek_evidence, expected_called_smartshield
):
    ...
    mock_detect_smartshield = mocker.patch(
        "modules.flowalerts.ssh.SSH.detect_successful_ssh_by_smartshield"
    )
    flow = SSH(
    ...
    with patch(
        "smartshield_files.common.smartshield_utils.utils.get_original_conn_flow"
    ...
    assert mock_detect_smartshield.called == expected_called_smartshield
...
@patch("smartshield_files.common.parsers.config_parser.ConfigParser")
...
def test_detect_successful_ssh_by_smartshield():
    ssh = ModuleFactory().create_ssh_analyzer_obj()
    ...
    assert ssh.detect_successful_ssh_by_smartshield("twid", conn_log_flow, flow)
