...
from smartshield_files.common.markov_chains import Matrix
...
MODULE_DB_MANAGER = "smartshield_files.common.abstracts.imodule.DBManager"
DB_MANAGER = "smartshield_files.core.database.database_manager.DBManager"
...
class ModuleFactory:
    ...
    def create_db_manager_obj(
        ...
        with (
            ...
            patch(
                "smartshield_files.core.database.redis_db.database."
                "RedisDB._set_redis_options",
                return_value=Mock(),
            ),
            patch(
                "smartshield_files.core.database.redis_db.database.ConfigParser",
                return_value=conf,
            ),
    ...
    def create_main_obj(self):
        """returns an instance of Main() class in smartshield.py"""
    ...
    def create_arp_obj(self, mock_db):
        ...
        arp.print = Mock()
        arp.evidence_filter.is_smartshield_peer = Mock(return_value=False)
        return arp
    ...
    def create_checker_obj(self):
        ...
        mock_main.terminate_smartshield = Mock()
    ...
    def create_daemon_object(self, mock_db):
        with (
            patch("smartshield.daemon.Daemon.read_pidfile", return_type=None),
            patch("smartshield.daemon.Daemon.read_configuration"),
            patch("builtins.open", mock_open(read_data=None)),
        ):
            daemon = Daemon(MagicMock())
        daemon.stderr = "errors.log"
        daemon.stdout = "smartshield.log"
        daemon.stdin = "/dev/null"
        daemon.logsfile = "smartshield.log"
        daemon.pidfile_dir = "/tmp"
        daemon.pidfile = os.path.join(daemon.pidfile_dir, "smartshield_daemon.lock")
        daemon.daemon_start_lock = "smartshield_daemon_start"
        daemon.daemon_stop_lock = "smartshield_daemon_stop"
        return daemon
    ...
    def create_trust_db_obj(self, sqlite_mock):
        with (
            patch("smartshield_files.common.abstracts.isqlite.ISQLite._init_flock"),
            patch(
                "smartshield_files.common.abstracts.isqlite.ISQLite._acquire_flock"
            ),
        ):
            trust_db = TrustDB(
                logger=self.logger,
    ...
    def create_cesnet_obj(self, mock_db):
        ...
        cesnet.node_info = [
            {"Name": "TestNode", "Type": ["IPS"], "SW": ["smartshield"]}
        ]
    ...
    def create_metadata_manager_obj(self, mock_db):
        ...
        mock_attributes = {
            ...
            "args": MagicMock(
                output="/tmp/output",
                config="config/smartshield.yaml",
                filepath=MagicMock(),
            ),
            "conf": MagicMock(
