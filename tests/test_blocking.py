...
def test_init_chains_in_firewall():
    ...
    with patch("os.system") as mock_system, patch.object(
        ...
        # simulate smartshieldBlocking not in any chain
        ...
        mock_system.assert_any_call(
            f"{blocking.sudo} iptables -N smartshieldBlocking >/dev/null 2>&1"
        )
        ...
        expected_calls = [
            call(
                ...
                f"smartshieldBlocking >/dev/null 2>&1"
            ),
            call(
...
def test_is_ip_already_blocked():
    ...
    fake_output = "Chain smartshieldBlocking (1 references)\n  target     prot opt source               destination\n  REJECT     all  --  192.168.1.100        anywhere"
