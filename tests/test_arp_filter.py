...
def test_is_smartshield_peer(p2p_enabled, is_private, peer_trust, expected):
    ...
    with patch(
        ...
        assert arp.is_smartshield_peer("192.168.1.100") == expected
...
def test_is_self_defense(ip, our_ips, blocking, has_poisoner, expected):
    ...
    arp = ARPEvidenceFilter(conf=Mock(), smartshield_args=args, db=db)
    arp.our_ips = our_ips
...
@pytest.mark.parametrize(
    "is_smartshield_peer_return, is_self_defense_return, expected_result",
    [
        (False, False, False),
        (True, False, True),
        (False, True, True),
...
def test_should_discard_evidence_combines_both_checks(
    ...
    arp = ModuleFactory().create_arp_filter_obj()
    with patch.object(
        arp, "is_smartshield_peer", return_value=is_smartshield_peer_return
    ), patch.object(
