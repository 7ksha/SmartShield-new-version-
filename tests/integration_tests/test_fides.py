"""
This file tests 2 different config files other than smartshield' default config/smartshield.yaml
...
def message_receive(port):
    ...
    # subscribe to the "fides2smartshield" channel
...
def test_conf_file2(path, output_dir, redis_port):
    ...
    output_file = os.path.join(output_dir, "smartshield_output.txt")
    command = [
        "./smartshield.py",
        "-t",
        "-g",
    ...
    with open(output_file, "w") as log_file:
        ...
        print("SIGTERM sent. killing smartshield")
        os.kill(process.pid, 9)

    message_receive(redis_port)

    print(f"smartshield with PID {process.pid} was killed.")

    print("smartshield is done, checking for errors in the output dir.")
...
def test_trust_recommendation_response(path, output_dir, redis_port):
    ...
    Preparation:
        ...
        those peers, NOTE that Fides Module only asks for opinion from known
         peers
        - Run smartshield (includes Fides Module) in a thread and wait for all
         modules to start
    ...
    try:
        ...
        with open(output_file, "w") as log_file:
            ...
            print(f"Output and errors are logged in {output_file}")
            # these 12s are the time we wait for smartshield to start all the modules
            countdown(60, "test message")
            message_send(redis_port)
            # these 18s are the time we give smartshield to process the msg
