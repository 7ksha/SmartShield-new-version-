...
def test_messaging(
    ...
    smartshield to the network (~other peers).

    First smartshield instance is a general node in the network, its connection
     string is generated and extracted from logs as a normal user would do,
    in a very standard use case.

    The second instance of smartshield acts as a normal-user-peer that joins the
    network via the aforementioned smartshield instance,
    which extends the standard use case of connecting to such P2P network.
    """
    # Two smartshield instances are necessary to be run in this test.
    # Prepare output dir for the main smartshield instance.
    ...
    output_file_peer = os.path.join(output_dir_peer, "smartshield_output.txt")

    # this will be used for the extraction of the connection string form the
    # logs of iris running under the main smartshield
    ...
    smartshield_iris_main_config_file = (
        "tests/integration_tests/config/smartshield_iris_main.yaml"
    )
    modify_yaml_config(
        input_path="config/smartshield.yaml",
        output_dir=os.path.dirname(smartshield_iris_main_config_file),
        output_filename=os.path.basename(smartshield_iris_main_config_file),
        changes={
    ...
    with open(output_file, "w") as log_file:
        with open(output_file_peer, "w") as iris_log_file:
            ...
            # to the same file
            # command for the main smartshield instance
            command = [
                "./smartshield.py",
                "-t",
                "-g",
                ...
                str(output_dir),
                "-c",
                smartshield_iris_main_config_file,  # we're using the dafult peer
                # config here
                "-P",
                str(redis_port),
            ]
            ...
            # let smartshield properly and fully star with all of its parts and modules.
            countdown(80, "Sending msg in fides2network")
            # Sending a manual message to make sure there is an alert generated, because
            # is is highly probable that both smartshield have covered their network captures
            ...
            # these seconds are the time we give smartshield to process the msg
            ...
            print("Sending SIGKILL to the 2 instances of smartshield + iris")
            # Kill em. Without kindness.
            os.kill(process.pid, 9)
            print(f"smartshield with PID {process.pid} was killed.")

            os.kill(peer_process.pid, 9)
            print(f"smartshield peer with PID {peer_process.pid} was killed.")

    print("smartshield is done, checking for errors in the 2 output dirs.")
