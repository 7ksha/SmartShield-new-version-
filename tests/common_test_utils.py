...
def modify_yaml_config(
    input_path="config/smartshield.yaml",
    output_filename="updated_smartshield.yaml",
    output_dir=None,
    changes=None,
):
    """
...
def run_smartshield(cmd):
    """runs smartshield and waits for it to end"""
    smartshield = subprocess.Popen(cmd, stdin=subprocess.PIPE, shell=True)
    return_code = smartshield.wait()
    return return_code
...
def check_for_text(txt, output_dir):
    """function to parse smartshield_output file and check for a given string"""
    smartshield_output = os.path.join(output_dir, "smartshield_output.txt")
    with open(smartshield_output, "r") as f:
        for line in f:
            if txt in line:
                return True
    return False
...
def has_ignored_errors(line):
    ...
    indicate that something is wrong with smartshield code
...
def assert_no_errors(output_dir):
    """function to parse smartshield_output file and check for errors"""
    error_files = ("smartshield_output.txt", "errors.log")
