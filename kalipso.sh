#!/bin/bash
cd modules/kalipso
echo "To close all unused redis servers, run smartshield with --killall"
file="../../running_smartshield_info.txt"
...
if [[ ${#open_redis_servers[@]} -eq 0 ]]; then
  echo "You have 0 open redis-servers to use. Make sure you run smartshield first"
  exit 1
