...
exit_on_cmd_failure() {
  if [ $? -ne 0 ]; then
      echo "Problem installing smartshield. Aborting."
      exit 1
  fi
}
...
print_green "Installing smartshield dependencies ..."
...
# create a symlink to zeek so that smartshield can find it
ln -s /opt/zeek/bin/zeek /usr/local/bin/bro
export PATH=$PATH:/usr/local/zeek/bin
echo "export PATH=$PATH:/usr/local/zeek/bin" >> ~/.bashrc

# dont continue with smartshield installation if zeek isn't installed
...
print_green "Installing p2p4smartshield"
# build the pigeon and Add pigeon to path
git submodule init && git submodule update && cd p2p4smartshield && go build && export PATH=$PATH:$(pwd) >> ~/.bashrc && cd ..
...
print_green "Successfully installed smartshield."
