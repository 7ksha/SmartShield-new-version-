...
GSoC 2023 smartshield Final Report
...
## Overview
### Goal
This project aims to provide a robust and easy to use profiling framework for smartshield. The features created should have a simple interface and minimal involvement needed to get usable interactive data to find areas to improve the program.
...
#### Step 1
To start with, go to smartshield.yaml and make sure the feature is enabled.
...
If this setting is set to: “no” then only the main process that initiates smartshield gets tracked.
...
Next, just run smartshield as normal. For an example command, run:
``` ./smartshield.py -e 1 -f dataset/test12-icmp-portscan.pcap ```
...
this means that the profiler starts a recursive process to run the same command used to run smartshield but with the tracer context active. This is so that all the data can be captured.
...
#### Step 1
Go to smartshield.yaml and make sure the settings are set correctly.
...
#### Step 2
Run smartshield.py as normal. The output shouldn’t be any different than if this feature were disabled. The only difference should be
...
in smartshield.yaml, first, enable memory profiling
...
Now just run smartshield.py. You should see this at the start
...
Go to ```smartshield.yaml``` and use the following settings
...
### Step 2
Running smartshield.py should result in the following output
...
##### Step 3
Run smartshield.py like normal.
...
##### Step 1
As always, make sure the live memory profiler is enabled in smartshield.yaml.

##### Step 2
In smartshield.py, uncomment the following line in start() for the Main class.
```
...
##### Step 3
Just run smartshield.py and check if output is similar to below.
...
The CPUProfiler and MemoryProfiler classes are meant to be called by the main smartshield.py. Since the behaviors are different depending on the settings in smartshield.yaml, these top level profiling classes act as Factory Method class, which takes on the identity of either DevProfiler or LiveProfiler depending on the class constructor parameters.
CPUProfiler Class
