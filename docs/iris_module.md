...
The Iris Module needs a trust-based mediator with smartshield. This mediation and trust evaluation is provided by the Fides Module.


## Docker direct use
```
docker pull stratosphereips/smartshield
docker run -it --rm --net=host --interface=<select_interface_to_run_on> stratosphereips/smartshield
```

Both the Fides Module and the Iris Module can be enabled in smartshield' configuration file by adding:
...
smartshield needs to be running on an interface or a growing Zeek directory. The Iris Module is ignored when smartshield is run on a file.
...
Iris will be inactive by default because Fides is inactive by default in smartshield.

Please run the Fides Module or another trust evaluation module when running Iris.

Iris Module and Fides Module have been designed to cooperate and therefore can be both enabled in the configuration file of smartshield by adding:
...
Iris uses the internet to communicate with other peers and Redis channels of smartshield to communicate with a trust intelligence module.
...
### Bootstrapping node
The smartshield configuration file now has an option for starting as a bootstrapping node.
The bootstrapping functionality for the global P2P network under Iris is facilitated.
This mode triggers only if smartshield is run on an interface or growing zeek log directory mode AND the bootstrapping is set to True in the smartshield configurations file AND GlobalP2P mode is allowed.
When the bootstrapping mode is used, smartshield runs with a subset of nodes that are selected by names (currently Fides and Iris).
...
smartshield is written in Python in its entirety, while Iris is based on Go. And following the best practices of
...
It has been decided by the development team of smartshield that running the unit test of Iris
...
The first node (called main) under a smartshield instance is created and puts its connection string that contains its identity/public key and details of how to reach it
in the P2P network.
...
smartshield contains a log file for reports received by other peers and peer updates of the Iris Module in
```output/<path_to_a_smartshield_output_folder>/iris/iris_logs.txt```
