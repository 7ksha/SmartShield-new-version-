...
## Slack
smartshield uses the WebHook method to send data to Slack, more info [here](https://api.slack.com/messaging/webhooks).
...
5. You need to give your app the correct scope. smartshield only needs write access to one channel. Do:
...
9. Edit the config/smartshield.yaml file, put `slack` in the export\_to variable, and add the channel's name to which you want to send.

    [exporting_alerts]
    export_to = [slack]
    slack_channel_name = smartshieldAlertsChannel


## STIX

If you want to export alerts to your TAXII 2 server using STIX 2.1 format,
set ```export_to``` to ```stix``` and smartshield will automatically generate a
...
```use_https```: set to true to connect over HTTPS (be careful that the default TAXII server in smartshieldWeb, Medallion, do not support HTTPS yet)
...
```push_delay```: time between automatic pushes (in seconds) when smartshield is
...
smartshield stores the generated bundle for each run in the output directory of that
execution (for example `output/<run_id>/STIX_data.json`), so you can inspect the
exact STIX objects that were pushed.

If running on a file, smartshield will export once before shutdown.
If running on an interface, smartshield will export to the server every
```push_delay``` seconds (default 1 hour).

## JSON format


By default smartshield logs all alerts to ```output/alerts.json``` in [CESNET's IDEA0 format](https://idea.cesnet.cz/en/index) which is also a JSON format.

## CESNET Sharing

smartshield supports exporting alerts to warden servers, as well as importing alerts.

To enable the exporting, set ```receive_alerts``` to ```yes``` in config/smartshield.yaml

The default configuration file path is specified in the ```configuration_file``` variable in ```config/smartshield.yaml```
...
See the [difference between alerts and evidence](https://stratospherelinuxips.readthedocs.io/en/develop/architecture.html)) in smartshield architecture section.

You can change how often you get alerts (import) from warden server

By default smartshield imports alerts every 1 day, you can change this by changing the ```receive_delay``` value in ```config/smartshield.yaml```

smartshield logs all alerts to ```output/alerts.json``` in
[CESNET's IDEA0 format](https://idea.cesnet.cz/en/index) by default.

Make sure that the DigiCert_Assured_ID_Root_CA is somewhere accessible by smartshield. or run smartshield with
...
smartshield has logstash.conf file that exports our alerts.json to a given output file,
you can change the output to your preference (for example: elastic search, stdout, etc.)

## Text logs

By default, the output of smartshield is stored in the ```output/``` directory in two files:
...
smartshield supports exporting all the labeled flows and altflows stored in the sqlite database
...
this can be done by setting the ```export_labeled_flows``` parameter to ```yes``` in smartshield.yaml and changing
