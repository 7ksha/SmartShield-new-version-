...
# How to Create a New smartshield Module



## What is smartshield and why are modules useful
smartshield is a machine learning-based intrusion prevention system for Linux and MacOS, developed at the Stratosphere Laboratories from the Czech Technical University in Prague. smartshield reads network traffic flows from several sources, applies multiple detections (including machine learning detections) and detects infected computers and attackers in the network. It is easy to extend the functionality of smartshield by writing a new module. This blog shows how to create a new module for smartshield from scratch.

## Goal of this Blog
This blog creates an example module to detect when any private IP address communicates with another private IP address. What we want is to know if, for example, the IP 192.168.4.2, is communicating with the IP 192.168.4.87. This simple idea, but still useful, is going to be the purpose of our module. Also, it will generate an alert for smartshield to consider this situation. Our module will be called ```local_connection_detector```.

### High-level View of how a Module Works


All modules implement the ```IModule``` interface located at ```smartshield_files/common/abstracts/module.py```
Abstract methods in this interface must be implemented in every new smartshield module, the rest are optional.
...
smartshield is running so that the module doesn't terminate.
...
All this common print() does is acts as a proxy between the module responsible for printing, ```output.py```, and all smartshield modules.
...
- Saving a model before smartshield stops
- Saving alerts in a .txt file if the module's job is to export alerts
- Telling the main module (smartshield.py) that the module is done processing so smartshield.py can kill it
etc.


## Creating a Module
When smartshield runs, it automatically loads all the modules inside the ```modules/``` directory. Therefore,
our new module should be placed there.

smartshield has a template module directory that we are going to copy and then modify for our purposes.
...
Each module in smartshield should have a name, author and description.
...
So now everytime smartshield sees a new flow, you can access it from your module using the
following line

```python
msg = self.get_msg('new_flow')
```
the implementation of the ```get_msg()``` is placed in the abstract module in ```smartshield_files/common/abstracts/module.py```
...
smartshield requires certain info about the evidence to be able to deal with them.

First, since we are creating a new type of evidence that is not defined in the ```EvidenceType``` Enum in
```smartshield_files/core/structure/evidence.py```, we need to add a new type there.

so the ```EvidenceType``` Enum in ```smartshield_files/core/structures/evidence.py``` would look something like this
...
Now we need to use the Evidence structure of smartshield, to do that,
First import the necessary dataclasses

```python
from smartshield_files.core.evidence_structure.evidence import \
    (
        Evidence,
        ProfileID,
        TimeWindow,
...
First we start smartshield by using the following command:

```bash
./smartshield.py -i wlp3s0 -o local_conn_detector
```
...
Due to the high modularity of smartshield, adding a new smartshield module is as easy as modifying a few lines in our
template module, and smartshield handles running
...
smartshield currently have. You can enhance them, add detections, suggest new ideas using
...
Detailed explanation of [smartshield profiles and timewindows here](https://idea.cesnet.cz/en/classifications)
...
so if smartshield starts with sudo and the module doesn't need the root permissions, we drop them.
...
The messages received in the channel are flows the smartshield read by the input process.


## Reading Input flows from an external module (Advanced)

smartshield relies on input process for reading flows, either from an interface, a pcap, or zeek files, etc.
...
```config/smartshield.yaml``` file.
- Check that the \_\_init\_\_.py file is present in module directory
- Read the output files (errors.log and smartshield.log) - if there were any errors
...
the channel, make sure that:
    ...
    - The channel name is present in the supported_channels list in ```smartshield_files/core/database/redis_db/database.py```

### Testing


smartshield has 2 kinds of tests, unit tests and integration tests.
...
1- Make sure you're in smartshield main dir

2- Run all tests ```./tests/run_all_tests.sh```

smartshield supports the -P flag to run redis on your port of choice. this flag is
used so that smartshield can keep track of the ports it opened while testing and close them later.

### Adding your own unit tests

smartshield uses ```pytest``` as the main testing framework, You can add your own unit tests by:
...
4- go to the main smartshield dir and run ```./tests/run_all_tests.sh``` and every test file in the ```tests/``` dir will run
...
Adding a new feature to smartshield is an easy task. The template is ready for everyone to use and there is not much to learn about smartshield to be able to write a module.

If you wish to add a new module to the smartshield repository, issue a pull request and wait for a review.
