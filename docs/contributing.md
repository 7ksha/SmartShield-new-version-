...
These are a set of guidelines for contributing to the development of smartshield [1].

## How can you contribute?

* Run smartshield and report bugs and needed features, and suggest ideas
* Pull requests with a solved GitHub issue and new feature
* Pull request with a new detection module.

## Persistent Git Branches

The following git branches in the smartshield repository are permanent:

- `master`: contains the stable version of smartshield, with new versions at least once a month.
- `develop`: contains the latest unstable version of smartshield and also its latest features. All new features should be based on this branch.
...
to follow when contributing to smartshield:
...
## What branch should you base your contribution to smartshield?
...
- Each PR to develop will trigger the develop Github checks, these checks will run smartshield unit tests and integration tests locally in a ubuntu VM and in docker to make sure the branch is ready to merge.
- PRs won't be merged unless the checks pass.
- The cleaner you code/change/changeset is, the faster it will be merged.

## Beginner tips on how to create a PR in smartshield

Here's a very simple beginner-level steps on how to create your PR in smartshield

1. Fork the smartshield repo
2. Clone the forked repo
3. In your clone, checkout origin/develop: ```git checkout origin develop```
4. Install smartshield pre-commit hooks ```pre-commit install```
5. Generate a baseline for detecting secrets before they're committed ```detect-secrets scan --exclude-files ".*dataset/.*|(?x)(^config/local_ti_files/own_malicious_JA3.csv$|.*test.*|smartshield.yaml|.*\.md$)" > .secrets.baseline```
...
11. Open a PR in smartshield, remember to set the base branch as develop.
...
### How does smartshield work?

<img src="https://raw.githubusercontent.com/stratosphereips/StratosphereLinuxIPS/develop/docs/images/smartshield_workflow.png" title="smartshield Workflow">

* smartshield.py is the entry point, it’s responsible for starting all modules, and keeping smartshield up until the analysis is finished.

* smartshield.py starts the input process, which is the one responsible for reading the flows from the files given to smartshield using -f it detects the type of file, reads it and passes the flows to the profiler process. if smartshield was given a PCAP or is running on an interface , the input process starts a zeek thread that analyzes the pcap/interface using smartshield’ own zeek configuration and sends the generated zeek flows to the profiler process.

* smartshield.py also starts the update manager, it updates smartshield local TI files, like the ones stored in smartshield_files/organizations_info and smartshield_files/ports_info. later, when smartshield is starting all the modules, smartshield also starts the update manager but to update remote TI files in the background in this case.

* Once the profiler process receives the flows read by the input process, it starts to convert them to a structure that smartshield can deal with. it creates profiles and time windows for each IP it encounters.
...
* Profiler process stores the flows, profiles, etc. in smartshield databases for later processing. the info stored in the dbs will be used by all modules later. smartshield has 2 databases, Redis and SQLite. it uses the sqlite db to store all the flows read and labeled. and uses redis for all other operations. the sqlite db is created in the output directory, meanwhite the redis database is in-memory. 7-8. using the flows stored in the db in step 6 and with the help of the timeline module, smartshield puts the given flows in a human-readable form which is then used by the web UI and kalipso UI.
...
* now that we’re sure that the evidence isn’t whitelisted, the evidence process logs it to smartshield log files and gives the evidence to all modules responsible for exporting evidence. so, if CEST, Exporting modules, or CYST is enabled, the evidence process notifies them through redis channels that it found an evidence and it’s time to share the evidence.
...
* if p2p is enabled in config/smartshield.yaml, the p2p module shares the IP of the attacker, its’ score and blocking requests sent by the evidence process with other peers in the network so they can block the attackers before they reach them.

* The output process is smartshield custom logging framework. all alerts, warnings and info printed are sent here first for proper formatting and printing.



### What is the recommended development environment?

For minimum hassle when developing it's recommended to use ubuntu, install smartshield natively, and use your favorite IDE

### While developing, my module is not working and there's no errors shown to the CLI or printed to errors.log

Always make sure to run smartshield with -e 1, for example
    ./smartshield.py -e 1 -f <some_pcap> -o some_output_dir
...
- We use SQLite for storing all the flows and altflows, so if you want to store or retreive something you will most probably find the function you need already implemented there ( in smartshield_files/core/database/sqlite_db/database.py)
...
- If you run smartshield without any special arguments, smartshield starts redis cache db ( redis server port 6379 db 1) and Redis main db (redis port 6379 db 1)
- You can start smartshield with -m, which starts redis on a random available redis port in the range (32768 to 10000), or -P if you want to start redis on a specific port.
- smartshield starts the redis server if it's not started by default.
- smartshield uses its own redis.conf, it doesn't use the default one. you can find it in config/redis.conf.
- The cache db is shared among all running smartshield instances, and is persistent, meaning it is not deleted on each run unlike the main redis db (redis port 6379 db 1), which is overwritten every run.
- If you're gonna add a new redis channel to smartshield, remember to add it to the list of supported_channels in smartshield_files/core/database/redis_db/database.py


### How are the modules loaded?

- All modules in the modules/ directory that implement the IModule interface are automatically imported by smartshield, for more technical details check the load_modules() function in managers/process_manager.py


### There's some missing code in all modules, what's happening?

- All modules implement the IModule interface in smartshield_files/common/abstracts/module.py, it ensures that all modules behave the same, for example they all shutdown the same, they all keep track of the redis channels they're using, they all have a common __init__(), they all forward msgs to the printer in the same way, etc.
- Any logic that will be duplicated accross all modules should be in this interface


### How does smartshield stop?
...
<img src="https://raw.githubusercontent.com/stratosphereips/StratosphereLinuxIPS/develop/docs/images/how_smartshield_stops.jpg"
...
Redis database of smartshield, are strings, integers and floats grouped into custom dataclasses. Aforementioned data classes can
...
The Fides Module is designed to cooperate with a global-peer-to-peer module. The communication is done using smartshield' Redis
...
### **Communication**
The module uses smartshield' Redis to receive and send messages related to trust intelligence,
evaluation of trust in peers and alert message dispatch.

**Used Channels**
modules/fidesModule/messaging/message_handler.py
| **smartshield Channel Name** | **Purpose**                                                             |
|-----------------|-------------------------------------------------------------------------|
| `smartshield2fides`   | Provides communication channel from smartshield to Fides                      |
| `fides2smartshield`   | Enables the Fides Module to answer requests from smartshield2fides            |
...
|:-------------------------------:|-----------------|-----------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|
|             `alert`             | `smartshield2fides`   | FidesModule as self.__alerts.dispatch_alert(target=data['target'], confidence=data['confidence'],score=data['score']) | Triggers sending an alert to the network, about given target, which smartshield believes to be compromised. |
|     `intelligence_request`      | `smartshield2fides`   | FidesModule as self.__intelligence.request_data(target=data['target'])                                                | Triggers request of trust intelligence on given target.                                               |
