...
Example:
```commandline
# smartshield computed value of the flow
...
Blocking in smartshield is done for any IP that results in an alert. If an IP is detected as malicious and is blocked,
...
Blocking is disabled by default. To enable blocking in smartshield, start smartshield with the ```-p``` flag.
...
smartshield has a complex system to deal with Threat Intelligence feeds. The Threat Intelligence Module in smartshield is designed to enhance detection capabilities, utilizing both external and internal thread intelligence sources.

smartshield supports various indicators of compromise (IoCs) from Threat Intelligence (TI) feeds, including IPs, IP ranges, domains, JA3 hashes, and SSL hashes. This provides comprehensive coverage against potential threats

While file hashes and URLs aren't supported in TI feeds directly, smartshield compensates by integrating with specialized external services for these types of IoCs.

### External Threat Intelligence Services

Besides searching 40+ TI files for every IP/domain smartshield encounters, smartshield integrates with the following external threat intelligence services to enrich its detection capabilities:

**URLhaus**: This service is utilized for checking URLs observed in `http.log` and files observed in `files.log` against known malicious URLs and files. URLhaus provides a comprehensive database of malicious URLs, which smartshield queries to determine if observed URLs or files are associated with known malware or phishing campaigns.

**Spamhaus**: Spamhaus is used for inbound traffic IP lookups to assess the reputation of IP addresses encountered during the analysis. By querying Spamhaus, smartshield can identify IP addresses associated with spamming activities, botnets, and other malicious behaviors, enhancing its ability to detect and alert on suspicious network traffic.

**Circl.lu**: Circl.lu's service is leveraged for hash lookups, particularly for downloaded files. Each file hash extracted from `files.log` is checked against Circl.lu's extensive database of known malicious file hashes. This integration allows smartshield to identify and react to the transfer or presence of known malicious files within the monitored network environment.

Circllu returns scores (`hashlookup:trust`) for each md5. this score ranges from 0 to 100, with 0 being the most malicious.
This score is converted to a value that smartshield can deal with using the following equation
```python
...
**URLhaus Access**:
  ...
  - **Method**: smartshield queries the URLhaus API with URLs and file hashes observed in network traffic logs.
  - **Response Handling**: If a URL or file is found in the URLhaus database, smartshield generates an alert indicating the presence of a known threat.
...
**Spamhaus Access**:
  ...
  - **Response Handling**: smartshield interprets the DNSBL response to determine if an IP address is associated with known malicious activities, triggering alerts accordingly.
...
from the blacklists defined in ```ti_files``` path in the configuration file ```config/smartshield.yaml```.
...
smartshield also sets evidence about DNS answers of blacklisted domains. If test.com is blacklisted, and test.com resolves to 1.2.3.4, smartshield sets
...
You can add your own SSL feed by appending to the ```ja3_feeds``` key in ```config/smartshield.yaml```
...
You can add your own SSL feed by appending to the ```ssl_feeds``` key in ```config/smartshield.yaml```


### Matching of ASNs

Every time smartshield sees a new IP, it stores info about it in the db, for example its organization, RDNs, and ASN.
If the ASN of an IP matches a blacklisted ASN, smartshield alerts.
...
To avoid false positive "Malicious downloaded file" detections, before looking up MD5 hashes of each downloaded file online, smartshield checks if the given hash is part of a known FP.
...
If the hash is a part of that list, smartshield doesn't look it up.

### Adding your own remote feed


We update the remote ones regularly. The list of remote threat intelligence
files is set in the path of ```ti_files``` variable in ```config/smartshield.yaml```.
...
By default, the files are stored in the smartshield directory ```modules/ThreatIntelligence1/remote_data_files/```
are deleted after smartshield is done reading them.


Domains found in remote feeds are considered invalid, and therefore discarded by smartshield,
...
then everytime we start smartshield. However, they are updated automatically by
the update manager if they were changed on disk.

Only one smartshield instance is allowed to be using the update manager at a time to avoid race conditions.

By default, smartshield starts without the TI files, and runs the Update Manager in the background
if the ```wait_for_TI_to_finish``` option in smartshield.yaml is set to yes, smartshield will not start until the update manager is done

 and all TI files are loaded successfully,
this is useful if you want to ensure that smartshield doesn't miss
the detection of any blacklisted IPs, but it adds some time to the startup of smartshield
...
smartshield updates this database by default every 2 weeks using the following online db
...
```mac_db_update``` in ```config/smartshield.yaml```.
...
smartshield imports 100 alerts from warden servers each day, and automatically stores the IoCs in our database
...
These are the categories smartshield imports:
...
smartshield detects downloads of files from pastebin through HTTP with size >= 700 bytes.
...
When smartshield sees an HTTP unencrypted traffic in zeek's http.log it generates
an evidence with threat_level low


### Non-HTTP connections on port 80

smartshield detects established connections on port 80 that are not using HTTP
...
smartshield makes sure no matching flows were detected as HTTP by zeek
within 5 mins before or after the given flow. if not, smartshield sets an evidence saying
...
smartshield considers an IP performing a vertical port scan if it contacts 5 or more different destination ports to the same destination IP in at least one time window (usually 1hs). The flows can be both, Non-Established TCP or UDP flows. On each arriving flow this check is performed.
...
To minimize false positives, smartshield ignores the broadcast IP 255.255.255.255 and the multicast IP if it's the source of vertical port scan.


### Horizontal port scans

smartshield detects TCP and UDP horizontal port scans. It considers an IP performing a horizontal port scan if it contacted 6 or more destination IPs on the same port with not established connections.
...
To minimize false positives, smartshield ignores the broadcast IP 255.255.255.255 if it's the source or the destination of horizontal port scans, and ignores all resolved IPs if they're the destination of port scans.


### PING Sweeps

ICMP messages can be used to find out which hosts are alive in a network.
smartshield relies on Zeek detections for this, but it is done with our own Zeek scripts located in
...
smartshield detects when an IP is requesting 4, 8, 12, etc. different IPs from the DHCP server within the same
