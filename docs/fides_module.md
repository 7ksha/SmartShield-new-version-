...
smartshield implements an internet global P2P system for Threat Intelligence sharing and alerting.

The Fides module implements the Global P2P system in smartshield.
...
If you plan on using the Fides Module, please be aware that it is used only if smartshield is running on an interface OR on a growing Zeek directory. The `--use_fides=True` is ignored when smartshield is run on a file.
...
Fides is inactive by default in smartshield.

To enable it, change ```use_fides=False``` to ```use_fides=True``` in ```config/smartshield.yaml```.

And start smartshield on your interface.
...
smartshield contains a minimal log file for reports received by other peers and peer updates in the ```output``` directory if not manually specified using the appropriate smartshield parameter upon start.
...
smartshield only shares the trust level and confidence values generated by smartshield about IPs to the network, no more information.
