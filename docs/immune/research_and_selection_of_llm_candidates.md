...
As part of this effort, nine targeted unit tests were developed to evaluate the key capabilities required for integration into the smartshield immune architecture. All evaluations were conducted on a computer based on the x64 architecture. Evaluation on Raspberry Pi (RPI) hardware will be carried out in future tasks, and the results will be contrasted accordingly. 
...
A special repository for storing the tests and other utilities used for evaluating smartshield IMMUNE is available [here](https://github.com/stratosphereips/smartshield-tools/). The complete procedure for running the tests is described [here](https://github.com/stratosphereips/smartshield-tools/blob/main/llm-unittest/README.md).
...
\[2\] [Repository](https://github.com/stratosphereips/smartshield-tools/tree/main/llm-unittest) with **promptfoo** unittest and the scripts used for generating the heatmap
