...
- [Acceptable performance benchmarks](#acceptable-performance-benchmarks)
  * [Current Benchmarks](#current-benchmarks-732025)
    ...
    + [smartshield on RPI Benchmarks](#smartshield-on-rpi-benchmarks)
...
- ARM processor:
  ...
  - Many applications including smartshield are designed for x86 architecture, so running them on ARM requires emulation or translation layers, which can reduce performance.
...
To be able to determine whether smartshield is going to run with full capabilities on RPI 5 or not, First, we need to know how smartshield behaves under normal conditions. This is our baseline benchmark.

**Testing Setup:**

- Testing will be done using docker, since this is the recommended way to run smartshield on computers and on RPIs, and also to be able to introduce limitations.

- smartshield supports reading zeek logs, PCAPs, netflow files, and many different formats. We chose PCAPs to be able to mimic real-life scenarios and control the speed of the flows smartshield receives to be able to benchmark different flow speeds and network loads.
...
Since this is a very powerful server, we decided to limit some of the resources to see how smartshield behaves on average computers. These limits are added as limits to the docker container that’s used to run smartshield.
...
--memory-swap=8g
--shm-size=512m
stratosphereips/smartshield:latest /bin/bash
...
* All the given PCAPS are capturing 24 hs worth of traffic and passing it to smartshield in minutes, the intensity of the traffic is usually less resource intensive in everyday use.
...
- smartshield is shipped with its own performance profilers done by one of our GSoC contributors. However, the CPU profilers included in smartshield are for pinpointing functions that take the most CPU time, which is not what we’re interested in in for benchmarking. And the memory profiler profiles each smartshield module separately, and doesn’t profile smartshield as a whole. So there’s no graph showing RAM usage that is aggregated for all processes.

- We developed our own CPU and RAM profilers for benchmarking smartshield.
...
* CPU usage of smartshield is captured every 5 minutes in this graph.
...
- In Normal and Malicious traffic, smartshield uses less than 100% of 4 cores. This is the 350% in the graph. Each 100% represents full usage of 1 core.

- smartshield takes longer time and uses more CPU the more malicious the traffic gets, because malicious flows trigger more logic in smartshield detection modules, branches more, and does more database calls for storing and tracking suspicious activity.

  - smartshield analyzed the normal PCAP (CTU-Normal-6) in 14 minutes, but took 21 hours and 33 minutes for the similarly sized malicious PCAP (CTU-Malware-Capture-Botnet-327-2) due to the different traffic types.

  - smartshield taking longer to analyze malicious PCAPs is expected.
...
- The average RAM usage across the four experiments was around 2 GBs, an acceptable number considering the typical RAM capacity of modern computers. It’s also reasonable given that smartshield simultaneously runs two machine learning models and twelve detection/threat intelligence models in Python.
...
|               PCAP               |  Size  | Type of traffic | Total Disk Usage | Zeek log files | smartshield logs and databases | Total number of Zeek flows |
| :------------------------------: | :----: | :-------------: | :--------------: | :------------: | :----------------------: | :------------------------: |
...
## smartshield on RPI Benchmarks

- For these experiments, smartshield is running in docker on the Raspberry Pi.
...
- CPU spikes at the beginning are expected due to how smartshield processes the given flows. As soon as smartshield starts, it reads all the given flows at once, and gradually processes them. These heavy I/O operations of reading the flows from disk at the beginning followed by categorizing, profiling and passing them to detection modules may not be the main cause of the CPU spikes, but contributes greatly to them.

- Clock speed reduction or some form of performance degradation for a short period of time (until the CPU spikes are flattened) may happen in the RPI if other resource-intensive programs are run alongside smartshield.

- Whether the Raspberry Pi can simultaneously run other resource-intensive applications alongside smartshield remains under investigation and depends on the performance of smartshield' new ML models.
...
- RAM usage in smartshield typically remains stable until it terminates.
...
- smartshield uses Redis as its main cache for flow-related data. While we're working to reduce memory usage by invalidating more cache entries, it might not be our top priority given smartshield' acceptable performance on the RPI.


**Conclusion**

- While the RAM usage indicates capacity for running programs alongside smartshield on the RPI, the CPU usage suggests that doing so could negatively impact performance of smartshield and the other running programs.
...
|               PCAP               | Size  | Type of traffic | Total Disk Usage | Zeek log files | smartshield logs and databases |
| :------------------------------: | ----- | --------------- | ---------------- | -------------- | ------------------------ |
...
- Log rotation of zeek logs, smartshield logs and smartshield SQLite database is a necessity if smartshield is going to be running non-stop on the RPI’s limited SD card.
...
|               PCAP               |  Size  | RPI Disk Usage | Normal Computer Disk Usage | RPI Zeek Logs | Normal Computer Zeek Logs | RPI smartshield Logs | Normal Computer smartshield Logs |
| :------------------------------: | :----: | :------------: | :------------------------: | ------------- | :------------------------: | -------------- | :------------------------: |
...
In conclusion, smartshield is able to run on the Raspberry Pi . In fact, the performance results not only confirmed its viability on the platform but also exceeded our expectations.
