# Network Event Summarization Dataset for smartshield IDS
...
Develop a dataset for network security event summarization to be integrated with the smartshield Immune system, optimized for deployment on low-resource hardware such as the Raspberry Pi 5. This dataset will be used to fine-tune compact language models capable of generating concise and actionable summaries of security incidents from raw smartshield alert data, enabling real-time threat analysis in resource-constrained environments.

The current version of the dataset used for finentuning LLM models is available [here](https://github.com/stratosphereips/smartshield-tools/raw/refs/heads/main/alert_summary/datasets/summarization_dataset_v3.json.gz)
...
### Scope Constraints
- **Alert Format**: Analysis currently limited to smartshield alert format; generalization to other IDS outputs requires format adaptation
...
The complete set of scripts for creating and updating the dataset is availabble in the [smartshield tools repository](https://github.com/stratosphereips/smartshield-tools/tree/main/alert_summary).

### Stage 1: Incident Sampling
Extract security incidents from smartshield `alerts.json` logs with category labels (Malware/Normal):
...
```
Raw smartshield Logs (alerts.json)
         ↓
