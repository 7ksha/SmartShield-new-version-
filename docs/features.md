...
section and a brief description of how smartshield works.
...
This threshold can be changed ```config/smartshield.yaml``` by changing the value of  ```long_connection_threshold```

### Incompatible CN

Zeek logs each Certificate CN in ssl.log

When smartshield enccounters a cn that claims to belong to any of smartshield supported orgs (Google, Microsoft, Apple or Twitter)
smartshield checks if the destination address or the destination server name belongs to these org.

If not, smartshield generates an alert.


### CN URL Mismatch

Zeek logs each Certificate CN in ssl.log
For each CN smartshield encounters, it checks if the server name is the same as the CN
Or if it belongs to the same org as the CN. if not, smartshield triggers an evidence



### High entropy DNS TXT answers

smartshield check every DNS answer with TXT record for high entropy
strings.
Encoded or encrypted strings with entropy higher than or equal 5 will then be detected using shannon entropy
and alerted by smartshield.

the entropy threshold can be changed in smartshield.yaml by changing the value of ```entropy_threshold```



### Devices changing IPs

smartshield stores the MAC of each new IP it sees in conn.log.

Then for every source address in conn.log, smartshield checks if the MAC of it was used by another IP.
...
smartshield uses zeek tunnel.log to alert on GRE tunnels when found. Whenever one
any action other than "Tunnel::DISCOVER" is found, smartshield sets an evidence
with threat level low

## GRE tunnel scans

smartshield uses zeek tunnel.log to alert on GRE tunnels scan. smartshield considers any log with "Tunnel::DISCOVER" action a GRE scan.
...
smartshield alerts when 3+ invalid SMTP login attempts occurs within 10s


### Connection to private IPs


smartshield detects when a private IP is connected to another private IP with threat level info.
...
smartshield detects the currently used local network and alerts if it find a
...
and smartshield sees a forged packet going from 192.168.1.2 to 10.0.0.1, it will alert

smartshield detects the current local network by using the local network of the private
ips specified in ```client_ips``` parameter in ```smartshield.yaml```

If no IPs are specified, smartshield uses the local network of the first private source ip
...
smartshield ignores evidence of this type when the destination IP is a private IP outside of local network and is
communicating on port 53/UDP. smartshield marks that destination address as the DNS server when 5 flows are seen using port
53/udp while having DNS answers. this is likely a DNS misconfiguration hence a FP.


### Weird HTTP methods

smartshield uses zeek's weird.log where zeek logs weird HTTP methods seen in http.log

When there's a weird HTTP method, smartshield detects it as well.

### Non-SSL connections on port 443

smartshield detects established connections on port 443 that are not using SSL
using zeek's conn.log flows

if smartshield finds a flow using destination port 443 and the 'service' field
...
5 minutes later as non-SSL. smartshield detects that and does not set an evidence for any of them.
...
smartshield detects established connections on port 80 that are not using SSL
using zeek's conn.log flows.

if smartshield finds a flow using destination port 80 and the 'service' field
in conn.log isn't set to 'http', it sets and evidence.

If a flow without http as a service is found, smartshield first checks past and future flows from the
same src ip + dst ip + port to see if there's a flow with http as a service, if there is, smartshield ignores the alert.
...
If smartshield runs by capturing packets directly from a network device (as opposed to, for example, a PCAP file), this detection will ignore all connections that happen in the first 3 minute of operation of smartshield. This is because most times smartshield is started when the computer is already running, and many DNS connections were already done. So waiting 3 minutes decreases the amount of False Positives.
...
DNS resolutions of well known orgs might be done using DoH, in this case, smartshield
...
smartshield uses it's own lists of organizations info (IPs, IP ranges, domains, and ASNs) stored in ```smartshield_files/organizations_info``` to check
whether the IP/domain of each flow belong to a known org or not.

smartshield doesn't detect 'connection without DNS' when running
...
of the docs for info on how smartshield detects DoH.


### Successful SSH connections

smartshield detects successful SSH connections using 2 ways
...
smartshield doesn't detect 'DNS resolutions without a connection' when running
...
When running on interface and files. For each DNS flow found, smartshield waits 30 mins zeek time
...
flows is detected, smartshield sets the evidence.

To avoid accumulating so many pending DNS flows for 30 mins, smartshield checks if the connection of the pending DNS flows
arrived every 10 and 20 mins too, if not found, smartshield waits extra 10 mins (so that would be 30 mins total) and sets the
evidence.

### Connection to unknown ports

smartshield has a list of known ports located in ```smartshield_files/ports_info/services.csv```

and a list of ports that belong to a specific organization in ```smartshield_files/ports_info/ports_used_by_specific_orgs.csv```

These are the cases where smartshield marks the port as known and doesn't trigger an alert

1. If the port is in the list of well known ports in `services.csv`.
2. If smartshield has that port's info in `ports_used_by_specific_orgs.csv` and the source and destination addresses belong to that organization.


smartshield considers an IP belongs to an org if:
...
4. If the IP is hardcoded in any of the organizations IPs in `smartshield_files/organizations_info/`.

Otherwise, smartshield triggers and "unknown port" evidence.
...
smartshield generate a 'possible data exfiltration alerts when the number of uploaded files to any IP exceeds 700 MBs.

The number of MBs can be modified by editting the value of ```data_exfiltration_threshold``` in ```config/smartshield.yaml```

### Malicious JA3 and JA3s hashes

smartshield uses JA3 hashes to detect C&C servers (JA3s) and infected clients (JA3)

smartshield is shipped with it’s own zeek scripts that add JA3 and JA3s fingerprints to the SSL log files generated by zeek.

smartshield supports JA3 feeds in addition to having more than 40 different threat intelligence feeds.
The JA3 feeds contain JA3 fingerprints that are identified as malicious.
The JA3 threat intelligence feed used by smartshield now is Abuse.ch JA3 feed.
And you can add other JA3 TI feeds in ```ja3_feeds``` in ```config/smartshield.yaml```.
...
smartshield detects any connection to port 0 using any protocol other than 'IGMP' and 'ICMP' as malicious.


### Multiple reconnection attempts

Multiple reconnection attempts in smartshield are 5 or more not established flows (reconnections) to the same destination IP.



### Zeek alerts

By default, smartshield depends on Zeek for detecting different behaviours, for example
Self-signed certs, invalid certs, port-scans and address scans, and password guessing.

Some scans are also detected by smartshield independently of Zeek, like ICMP sweeps and vertical/horizontal portscans.
...
To detect DGA, smartshield will count the amount of NXDOMAINs met in the DNS traffic of each source IP.

Then we alert when there is 10 or more NXDOMAINs.

Every 10,15,20 ..etc smartshield generates an evidence.

### Connection to multiple ports

When smartshield encounters a connection to or from a specific IP and a specific port, it scans previous connections looking for connection to/from that same IP using a different port.

It alerts when finding two or more connections to the same IP.

### Malicious SSL certificates

smartshield uses SSL certificates sha1 hashes to detect C&C servers.

smartshield supports SSL feeds and is shipped with Abuse.ch feed of malicious SSL hashes by default.
And you can add other SSL feeds in ```ssl_feeds``` in ```config/smartshield.yaml```.

### Young Domains

smartshield uses whois python library to get the creation date of every domain met in the dns flows.

If a domain's age is less than 60 days, smartshield sets an alert.
...
smartshield uses zeek to detect SMTP connections,
When zeek detects a bad smtp login, it logs it to smtp.log, then smartshield reads
this file and sets an evidence.

### SMTP bruteforce

smartshield detects a SMTP bruteforce when 3 or more bad SMTP
logins happen within 10 seconds.

---

With every generated evidence, smartshield gathers as much info
...
smartshield gathers AS, hostname, SNI, rDNS and any available data about this IP and you get an alert saying:
...
Whenever smartshield sees a new domain in dns.log, if the domain ends with '.in-addr.arpa'
smartshield keeps trach of this domain and the source IP that made the DNS request.

Then, if the source IP is seen doing 10 or more ARPA queries within 2 seconds,
smartshield generates an ARPA scan detection.


### SSH version changing

Zeek logs the used software and software versions in software.log, so smartshield knows from this file the software used by different IPs,
like whether it's an SSH::CLIENT, an HTTP::BROWSER, or an HTTP::SERVER

When smartshield detects an SSH client or an SSH server, it stores it with the IP and the SSH versions used in the database

Then whenever smartshield sees the same IP using another SSH version, it compares the stored SSH versions with the current SSH versions

If they are different, smartshield generates an alert
...
smartshield detects this and sets an informational evidence.
...
smartshield is a behavioral-based IPS that uses machine learning to detect malicious behaviors in the network traffic. It is a modular software that can be extended. When smartshield is run, it spawns several child processes to manage the I/O, to profile attackers and to run the detection modules.

Here we describe what detection modules are run on the traffic to detect malicious behaviour.


Modules are Python-based files that allow any developer to extend the functionality of smartshield. They process and analyze data, perform additional detections and store data in Redis for other modules to consume. Currently, smartshield has the following modules:
...
<table>
  ...
  <tr>
    <td>Exporting</td>
    <td>Exports smartshield alerts to Slack servers and STIX servers.</td>
    <td>✅</td>
  </tr>
  <tr>
    <td>IP_Info</td>
  ...
  <tr>
    <td>flowalerts</td>
    <td>Finds malicious behaviours by analyzing only one flow. Now detects: self-signed certificates, TLS certificates which validation failed, vertical port scans detected by Zeek (contrary to detected by smartshield), horizontal port scans detected by Zeek (contrary to detected by smartshield), password guessing in SSH as detected by Zeek, long connection, successful ssh</td>
    <td>✅</td>
  </tr>
  <tr>
    <td>leak_detector</td>
...
The path of the file can be modified by changing the ```RiskIQ_credentials_path``` parameter in ```config/smartshield.yaml```
...
To enable blocking in smartshield, start smartshield with the ```-p``` flag.

This feature is only supported in linux using iptables natively and using docker.

### Exporting Alerts Module

smartshield supports exporting alerts to other systems using different modules (ExportingAlerts, CESNET sharing etc.)
...
Refer to the [Flowalerts section of the docs](https://stratospherelinuxips.readthedocs.io/en/develop/flowalerts.html) for detailed explanation of what smartshield detects and how it detects.

### Disabled alerts

All smartshield detections are turned on by default, You can configure which alerts you want to enable/disable in ```config/smartshield.yaml```

smartshield support disabling unwanted alerts, simply add the detection you want to disable in
the ```disabled_detections``` list and smartshield will not generate any alerts of this type.
...
smartshield has a complex system to deal with Threat Intelligence feeds.

smartshield supports different kinds of IoCs from TI feeds (IPs, IP ranges, domains, JA3 hashes, SSL hashes)
...
smartshield gets every IP it can find in the network and tries to see if it is in any blacklist.
...
#### Matching of Domains
smartshield gets every domain that can find in the network and tries to see if it is in any blacklist.
...
Once a domain is found, it is verified against the downloaded list of domains from the blacklists defined in ```ti_files``` in the configuration file ```config/smartshield.yaml```.
If an exact match is found, then an evidence is generated.

If an exact match is not found, then smartshield verifies if the verified domain is a
subdomain of any domain in the blacklist.


For example, if the domain in the traffic is _here.testing.com_,
smartshield first checks if the exact domain _here.testing.com_ is in any blacklist,
and if there is no match, it checks if the domain _testing.com_ is in any blacklists too.

#### Matching of JA3 Hashes

Every time smartshield encounters an TLS flow,
it compares each JA3 and JA3s with the feeds of malicious JA3 and alerts when
there’s a match.
smartshield is shipped with the Abuse.ch JA3 feed by default
You can add your own SSL feed by appending to the file defined by the ```ja3_feeds``` key in
```config/smartshield.yaml```, which is by default ```config/JA3_feeds.csv```

#### Matching of SSL SHA1 Hashes

Every time smartshield encounters an SSL flow, it tries to get the certificate hash from zeek ssl.log,
then it compares the hash with our list of blacklisted SSL certificates

smartshield is shipped with the Abuse.ch SSL feed by default,

You can add your own SSL feed by appending to the file defined by the ```ssl_feeds``` key in ```config/smartshield.yaml```, which is by default
```config/SSL_feeds.csv```

#### Local Threat Intelligence files

smartshield has a local file for adding IoCs of your own,
it's located in ```config/local_data_files/own_malicious_iocs.csv``` by default,
this path can be changed by changing ```download_path_for_local_threat_intelligence``` in ```config/smartshield.yaml```.
...
Refer to the [architecture section of the docs](https://stratospherelinuxips.readthedocs.io/en/develop/architecture.html) for detailed explanation of smartshield threat levels.
...
smartshield has a local file for adding JA3 hashes of your own,
...
We update the remote ones regularly. The list of remote threat intelligence files is set in the variables ```ti_files``` variable in config/smartshield.yaml. You can add your own remote threat intelligence feeds in this variable. Supported extensions are: .txt, .csv, .netset, ipsum feeds, or .intel.
...
Use ```;``` for commenting TI files in ```config/smartshield.yaml``` instead of ```#```.

Commented TI files (lines starting with ;) will be completely removed from our database.


The remote files are downloaded to the path set in the ```download_path_for_local_threat_intelligence```. By default, the files are stored in the smartshield directory ```modules/ThreatIntelligence1/remote_data_files/```


#### Commenting a remote TI feed

If you have a remote file link that you wish to comment and remove from the database
you can do so by adding ';' to the line that contains the feed link in ```config/smartshield.yaml```, don't use the '#'
...
To make sure smartshield is up to date with the most recent IoCs in all feeds,
all feeds are loaded, parsed and updated periodically and automatically by
smartshield every 24 hours, which requires no user interaction.

The 24 hours interval can be changed by changing the ```TI_files_update_period``` key in ```config/smartshield.yaml```

Update manager is responsible for updating all remote TI files (including SSL and JA3 etc.)


By default, local smartshield files (organization_info, ports_info, etc.) are
cached to avoid loading and parsing
them everytime we start smartshield. However, they are updated automatically by
...
smartshield is shipped with an offline database (GeoLite2) in ```databases/GeoLite2-ASN.mmdb```
...
smartshield is shipped with an offline database (GeoLite2) in ```databases/GeoLite2-Country.mmdb```
to search for Geolocation.

#### Mac Vendors

smartshield is shipped with an offline database ```databases/macaddress-db.json``` for
MAC address vendor mapping.

This database is a combination of 2 different online databases, but the format of them
is changed to a format smartshield understands and to reduce the size of the db.

smartshield gets the MAC address of each IP from dhcp.log and arp.log and then searches the offline
database using the OUI.

If the vendor isn't found in the offline MAC database,
smartshield tries to get the MAc using the online database https://www.macvendorlookup.com

The offline database is updated manually and shipped with smartshield, you can find it in
the ```databases/``` dir.

smartshield makes sure it doesn't perform duplicate searches of the same MAC Address either online, or offline.
...
By default, zeek doesn't generate and log ARP flows, but smartshield is shipped with it's
...
smartshield considers an IP performing an ARP scan if it sends 5
or more non-gratuitous ARP to different destination addresses in 30 seconds or less.

#### ARP to a destination IP outside of local network

smartshield alerts when an ARP flow is being sent to an IP outside of local network as it's a weird behaviour
...
smartshield detects when a MAC with IP A, is trying to tell others that now that MAC
...
set ```receive_alerts```  to ```yes``` in config/smartshield.yaml

smartshield imports 100 alerts from warden servers each day, and automatically stores the aleerts in our database


Time to wait before receiving alerts from warden server is 1 day by default, you can change this
by chaning the ```receive_delay``` in ```config/smartshield.yaml```
...
smartshield has a list of suspicious user agents, whenever one of them is found in the traffic, smartshield generates
and evidence.
...
smartshield uses and offline MAC address database to detect the type of device based on the MAC OUI.

First, smartshield store the MAC address and vendor of every IP it sees (if available)

Second, When smartshield encounters a user agent in HTTP traffic it performs an online
query to http://useragentstring.com to get more info about this user agent,
like the os type, name and browser.

Third, When smartshield has both information available (MAC vendor and user agent),
...
smartshield stores the MAC address and vendor of every IP it sees
...
smartshield detects this with 'low' threat level

#### Pastebin downloads

Some malware use pastebin as the host of their malicious payloads.

smartshield detects downloads of files from pastebin with size >= 700 bytes.

This value can be customized in smartshield.yaml by changing ```pastebin_download_threshold```

When found, smartshield alerts pastebin download with threat level low because not all downloads from pastebin are malicious.


#### Executable downloads

smartshield generates an evidence everytime there's an
executable download from an HTTP website.


### Leak Detector Module

This module work only when smartshield is given a PCAP
...
If you want to contribute, improve existing smartshield detection modules or implement your own detection modules, see section :doc:`Contributing <contributing>`.
...
smartshield checks both TCP and UDP connections for port scans.


smartshield considers an IP performing a vertical port scan if it scans 6 or more different
destination ports
...
smartshield checks both TCP and UDP connections for horizontal port scans.


smartshield considers an IP performing a horizontal port scan if it contacted more than 3
...
smartshield does this detection using smartshield' own zeek script located in
zeek-scripts/icmps-scans.zeek for zeek and pcap files and using the portscan module for binetflow files.

## Connections Made By smartshield

smartshield uses online databases to query information about many different things, for example (user agents, mac vendors etc.)

The list below contains all connections made by smartshield
...
By default, smartshield whitelists alerts from or to any of the above domains, witch means that if an alert was detected
to one of the above alerts, smartshield does not detect it assuming it's a false positive and the connection was made
internally by smartshield.
...
Ensembling in smartshield is done by the Evidence Process.
...
Then, smartshield uses the following equation to
get the score of each evidence

threat_level = threat_level * confidence

smartshield accumulates the threat level of all evidenc, then, it checks if the accumulated threat
level reached a certain threshold or not.

If the accumulated threat level reached the threshold specified in
```evidence_detection_threshold```, smartshield generates and alert.
If not, smartshield waits for the next evidence, accumulates threat levels, and checks again until the threshold is reached.


## Controlling smartshield Sensitivity

The threshold that controls smartshield sensitivity is determined
by the ```evidence_detection_threshold``` key in ```config/smartshield.yaml```,
by default it is set to ```0.25```.


This threshold is used in smartshield according to the following equation

threshold per width = detection_threshold * width / 60

For example, if you're using the default smartshield width 3600, the threshold used in smartshield will be
...
When the accumulated threat levels of all evidence detected in a timewindow exceeds 15, smartshield will generate an alert.

In simple terms, it means smartshield will alert when users get the equivalent of 1 alert per minute.
...
Here are more options
  - 0.08:  Use this threshold If you want smartshield to be super sensitive with higher FPR,
  ...
  - 0.43:  Use this threshold If you want smartshield to be insensitive.
         Using this means smartshield will need so many evidence to trigger an alert.
...
smartshield is shipped with it's own custom zeek scripts to be able to extend zeek functionality and
customize the detections

#### Detect DoH

In the ```detect_DoH.zeek``` script, smartshield has it's own list of ips that belong to dns/doh servers,

When smartshield encouters a connection to any IP of that list on port 443/tcp, it assumes it's a DoH connetion,

and times out the connection after 1h so that the connection won't take too long to appear in smartshield.
...
smartshield is shipped with its own tool for CPU Profiling, it can be found it ```smartshield_files/common/cpu_profiler.py```
...
Options to enable cpu profiling can be found under the [Profiling] section of the ```smartshield.yaml``` file.
...
```cpu_profiler_multiprocess``` can be set to "yes" or "no" and only affects the dev mode profiling. If set to "yes" then all processes will be profiled. If set to "no" then only the main process (smartshield.py) will be profiled.
...
### Memory Profiling
Memory profiling can be found in ```smartshield_files/common/memory_profiler.py```
