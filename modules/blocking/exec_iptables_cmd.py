...
def exec_iptables_command(sudo: str, action, ip_to_block, flag, options):
    ...
    action options:
      insert : to insert a new rule at the top of smartshieldBlocking list
      delete : to delete an existing rule
    """

    command = (
        f"{sudo} iptables --{action} smartshieldBlocking {flag} {ip_to_block} "
        f'-m comment --comment "smartshield rule" >/dev/null 2>&1'
    )
