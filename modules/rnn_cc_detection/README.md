# RNN Command and Control detection for smartshield
This is the code of the module plus training programs of the RNN neural network that detects command and control channels in smartshield.
...
![Stratosphere Linux IPS (smartshield) - Frame 1](https://github.com/stratosphereips/StratosphereLinuxIPS/assets/2458867/eba4021a-4c54-4523-9380-fb179526d808)
...
Once the model is saved in a new file, you can just replace the old one for this one, since smartshield is searching for a file called `rnn_model.h5`.
...
# Testing
During testing, or inference, smartshield loads the model and waits for new 'letters' to be reported in the corresponding Redis channel. The letter are the Stratosphere Letters as describe below. 

smartshield checks each new letter (each new flow) to see if there is a match with a C&C. This is very CPU intensive, but it is necessary to detect C&C as soon as they happen not force the user to wait until the end of the time windows to do a check. 
...
# Training
smartshield comes with a pre-trained model that we trained in the datasets shown in this folder. The datasets comes from many verified malware C&C connections that we have executed. The dataset also has normal connections that could be misdetected if not in included in the training. 
...
For now, smartshield only creates behavioral models for TCP flows, and not UDP.
