...
class smartshieldThreatIntelligenceDatabase(ThreatIntelligenceDatabase):
    """Implementation of ThreatIntelligenceDatabase that uses smartshield native
    storage for the TI."""
    ...
    def get_for(self, target: Target) -> Optional[smartshieldThreatIntelligence]:
        ...
        # dict of STI or None
        if out:
            out = smartshieldThreatIntelligence(**json.loads(out))
        else:
            out = self.sqldb.get_smartshield_threat_intelligence_by_target(target)
        return out

    def save(self, ti: smartshieldThreatIntelligence):
        self.sqldb.store_smartshield_threat_intelligence(ti)
