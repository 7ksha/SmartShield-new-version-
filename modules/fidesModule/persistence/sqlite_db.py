...
from ..model.threat_intelligence import smartshieldThreatIntelligence
...
class SQLiteDB:
    ...
    def __smartshield_log(self, txt: str) -> None:
        self.logger.output_line_to_cli_and_logfiles(
            {"verbose": 2, "debug": 0, "from": self.name, "txt": txt}
        )

    def get_smartshield_threat_intelligence_by_target(
        self, target: Target
    ) -> Optional[smartshieldThreatIntelligence]:
        """
        Retrieves a smartshieldThreatIntelligence record by its target.

        :param target: The target (IP address, domain, etc.) of the intelligence.
        :return: A smartshieldThreatIntelligence instance or None if not found.
        """
        query = """
        ...
        if rows:
            ...
            return smartshieldThreatIntelligence(
                score=score,
    ...
    def store_smartshield_threat_intelligence(
        self, intelligence: smartshieldThreatIntelligence
    ) -> None:
        """
        Stores or updates the given smartshieldThreatIntelligence object in the database based on the target.

        :param intelligence: The smartshieldThreatIntelligence object to store or update.
        """
        query = """
    ...
    def __connect(self) -> None:
        ...
        """
        self.__smartshield_log(f"Connecting to SQLite database at {self.db_path}")
        self.connection = sqlite3.connect(
            self.db_path, check_same_thread=False
        )

        if self.connection is None:
            self.__smartshield_log("Failed to connect to the SQLite database!")
    ...
    def __execute_query(
        ...
        """
        with SQLiteDB._lock:
            self.__smartshield_log(f"Executing query: {query}")
    ...
    def __save(self, table: str, data: dict) -> None:
        ...
        self.__smartshield_log(f"Saving data: {data} into table: {table}")
    ...
    def __delete(
        ...
        self.__smartshield_log(f"Deleting from table: {table} where {condition}")
        self.__execute_query(query, params)
    ...
    def close(self) -> None:
        ...
        """
        if self.connection:
            self.__smartshield_log("Closing database connection")
    ...
    def __create_tables(self) -> None:
        ...
        for query in table_creation_queries:
            self.__smartshield_log(f"Creating tables with query: {query}")
