# Data saved to smartshield

smartshield database expects a dictionary of data. The data from this module (for a given IP) has the following format: 
...
The `p2p4smartshield` field in the database will the report from the network. The report will have the IP address that is
...
## The channel from smartshield to Go `p2p_pygo`

smartshield sends data to go in json. The Json object has two fields: `message` and `recipient`. 
...
## The channel from Go to smartshield `p2p_pygo`
